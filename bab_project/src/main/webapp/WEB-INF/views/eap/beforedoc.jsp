...
<body>
    ...
    <script>
		...
		$(".page-item.num .page-link").click(function(event) {
			...
			$("#s_eap_content_box").load("<%=request.getContextPath()%>/eap/beforedoc?page="+pageNum);
		})
		...
		$(".page-item.pre .page-link").click(function(event){
			...
			$("#s_eap_content_box").load("<%=request.getContextPath()%>/eap/beforedoc?page="+num);
		...
		$("#s_search_btn").click(function() {
			...
			var check = "${empty beforeDoc }";
			var check2 = "${not empty beforeDoc }";
			
			var valueSelect = $("#select_search option:selected").val();
			var valueInput = $("#search_bar").val();
			
			$.ajax({
				url : "<%= request.getContextPath() %>/eap/beforedoc",
				type : "get",
