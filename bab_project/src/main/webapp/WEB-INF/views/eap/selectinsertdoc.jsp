...
<body>
		...
		<div style="padding: 30px;">
			...
			<div style="clear: both;">
				<table class="table table-hover" style="text-align: center; vertical-align: middle;">
				  ...
				  <tbody>
				  	<c:forEach items="${insertdoc }" var="i">
				...
				<c:if test="${empty insertdoc }">
					<div id="s_notice">기안한 문서가 없습니다.</div>
				</c:if>
			</div>
    ...
    <script>
		...
		$(".page-item.num .page-link").click(function(event) {
			...
			$("#s_eap_content_box").load("<%=request.getContextPath()%>/eap/selectinsertdoc?page="+pageNum);
		})
		...
		$(".page-item.pre .page-link").click(function(event){
			...
			$("#s_eap_content_box").load("<%=request.getContextPath()%>/eap/selectinsertdoc?page="+num);
		...
		$("#s_search_btn").click(function() {
			...
			var check = "${empty insertdoc }";
			var check2 = "${not empty insertdoc }";
			
			var valueSelect = $("#select_search option:selected").val();
			var valueInput = $("#search_bar").val();
			
			$.ajax({
				url : "<%= request.getContextPath() %>/eap/selectinsertdoc",
				type : "get",
				...
				success : function(result) {
					if(check == 'true') {
						$('#s_notice').replaceWith('<div style="text-align: center;">'
				                  ...
				                  + '<button class="btn btn-primary" onclick="inlist()">목록으로</button>'
				               + '</div>');
					} else if(check2 == 'true') {
